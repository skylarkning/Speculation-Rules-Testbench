...
test("server-renders the Prefetch Lab runner", async () => {
  const response = await render();
  assert.equal(response.status, 200);
  assert.match(response.headers.get("content-type") ?? "", /^text\/html\b/i);

  const html = await response.text();
  assert.match(html, /<title>Prefetch Lab · Firefox Speculation Rules<\/title>/i);
  assert.match(html, /See what a prefetched navigation actually did/);
  assert.match(html, /Prepare prefetch/);
  assert.match(html, /Run control/);
  assert.match(html, /list source, immediate eagerness/);
